...
function main() {
    ...
    const N_LOAVES_OF_EACH_PERSON: Array<number> = readLine().split(" ").map(Number);
    console.log(findMinLoavesToSatisfyRules(N_LOAVES_OF_EACH_PERSON));
}

    function findMinLoavesToSatisfyRules(nLoavesOfEachPerson: Array<number>): string {
        let minLoavesToSatisfyRules: number = 0;
