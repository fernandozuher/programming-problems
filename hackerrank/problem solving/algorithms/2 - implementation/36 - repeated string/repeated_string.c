...
unsigned long find_quantity_of_remaining_a(char* string, const unsigned long N_REMAINING_CHARACTERS);
...
char* read_a_string() {
    const unsigned long INPUT_STRING_MAX_SIZE = 103;
...
unsigned long find_quantity_of_a_in_repeated_string(char* string, const unsigned long N_CHARACTERS) {
    ...
    quantity_of_a *= N_STRINGS;
    quantity_of_a += find_quantity_of_remaining_a(string, N_CHARACTERS % SIZE_STRING);
...
unsigned long find_quantity_of_remaining_a(char* string, const unsigned long N_REMAINING_CHARACTERS) {
    ...
    for (unsigned long i = 0; i < N_REMAINING_CHARACTERS; i++)
        if (*(string++) == 'a')
            quantity_of_a++;
